...
import { Upload, FileSpreadsheet, Trash2, AlertTriangle, CheckCircle2, RefreshCw } from 'lucide-react'
...
export function BookManager({ books, onAddBook, onRemoveBook, loading }: Props) {
  ...
  const handleDuplicateAction = useCallback(
    async (info: DuplicateInfo, overwrite: boolean) => {
      if (overwrite) {
        await onAddBook(info.file, true)
      }
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500">
              合計 {books.reduce((a, b) => a + b.games.length, 0)} 局
            </span>
          </div>
          ...
          {books.map((book) => (
            <div
              key={book.id}
              className="flex items-center gap-2 card px-3 py-2.5 group"
            >
              <FileSpreadsheet className="w-4 h-4 text-emerald-400 shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-sm text-slate-200 truncate">{book.fileName}</p>
                <p className="text-xs text-slate-500">
                  {book.date ?? '日付不明'} ・ {book.games.length} 局 ・ {book.players.join(', ')}
                </p>
              </div>
